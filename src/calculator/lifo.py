"""
LIFO (Last In, First Out) lot selection.

Consumes the most recently acquired lots first.
Often produces more short-term gains, which can be beneficial for
loss harvesting or when recent cost basis is high relative to proceeds.

Note: LIFO is a recognized method for crypto under current IRS guidance,
though the IRS requires consistent application within a tax year.
"""
...
def consume_lifo(
    ...
    """
    Consume `amount` of `token` using LIFO order and return a Disposal.

    Most recently acquired lots are consumed first (largest acquisition_date first).

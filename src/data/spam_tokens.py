"""
Known spam token contract addresses and detection patterns.

Spam tokens are airdropped in bulk to inflate transaction counts and trick
users into interacting with malicious contracts.  They should be excluded
from tax calculations and tier billing.

Addresses are lowercase Ethereum mainnet unless noted.
"""
from __future__ import annotations

import re

# ── Known spam contract addresses (at least 30) ─────────────────────────────
# These are well-documented spam/scam token contracts on Ethereum mainnet.
KNOWN_SPAM_CONTRACTS: frozenset[str] = frozenset({
...
# ── Spam name patterns ──────────────────────────────────────────────────────
# Token names/symbols matching these patterns are highly likely spam.
SPAM_NAME_PATTERNS: list[re.Pattern[str]] = [
    re.compile(r"(?i)visit\s+\S+\.com"),       # "Visit SomeScam.com"
    re.compile(r"(?i)claim\s+(at|your|reward)"),  # "Claim at X" / "Claim your reward"
    re.compile(r"(?i)https?://"),               # URLs in token name
    re.compile(r"(?i)\.com\b"),                 # Domains in token name
    re.compile(r"(?i)\.xyz\b"),
    re.compile(r"(?i)\.io\b"),
    re.compile(r"(?i)\.org\b"),
    re.compile(r"(?i)\.net\b"),
    re.compile(r"(?i)free\s+airdrop"),          # "Free Airdrop"
    re.compile(r"(?i)^airdrop\b"),              # Token named "Airdrop..."
    re.compile(r"(?i)reward\s*token"),           # "RewardToken"
    re.compile(r"(?i)voucher"),                  # "Gift voucher"
    re.compile(r"(?i)\$\s*\d+\s*(reward|bonus|gift)"),  # "$500 Reward"
]

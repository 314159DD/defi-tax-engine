/**
 * LIFO (Last In, First Out) lot consumption.
 *
 * Consumes the most recently acquired lots first (sorted by acquisitionDate descending).
 * Often produces more short-term gains, which can be beneficial for loss harvesting
 * or when recent cost basis is high relative to proceeds.
 *
 * Ported from: src/calculator/lifo.py
 */
...
/**
 * Consume `amount` of a token using LIFO order.
 *
 * Most recently acquired lots are consumed first (largest acquisitionDate first).
 ...
 */
export function consumeLifo(
  lots: TaxLot[],
  amount: Decimal,
): LotConsumption[] {
  const EPSILON = new Decimal('0.00000001');

  // Sort newest first (descending acquisitionDate). Stable sort preserves insertion order for ties.
